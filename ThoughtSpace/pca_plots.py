...
def create_observation_id(df: pd.DataFrame, idno_col: str) -> pd.DataFrame:
    """
    Function to create join key (observation ID) and set as index
    """
    df["probenum"] = df.groupby(idno_col).cumcount() + 1
    df
...
def esq_cols(df: pd.DataFrame, start_col: str, end_col: str) -> tuple:
    """
...
def z_score(
    ...
    for c in cols:
        ...
        if by_condition and by_sample:
            df["Z{}".format(c)] = df.groupby([condition_col, sample_col])[c].transform(
        ...
        # if by_condition set to true and by_sample set to false, apply z-score by condition only to all samples
            ...
            df["Z{}".format(c)] = df.groupby(condition_col)[c].transform(
                lambda x: zscore(x, ddof=1, nan_policy="omit")
            )
        # if by_sample set to true and by_condition set to false, apply z-score by sample only to all conditions
            ...
            df["Z{}".format(c)] = df.groupby(sample_col)[c].transform(
                lambda x: zscore(x, ddof=1, nan_policy="omit")
            )
        # if both set to false, apply Z-score to all
        elif by_person:
            df["Z{}".format(c)] = df.groupby([person_col])[c].transform(
                lambda x: zscore(x, ddof=1, nan_policy="omit")
            )
        else:
...
def naive_pca(input_dict: Dict[str, pd.DataFrame]) -> OrderedDict[str, PCA]:
    """
...
def varimax(
    Phi: np.ndarray, gamma: float = 1, q: int = 20, tol: float = 1e-6
) -> np.ndarray:
    """
    ...
    (
        p,
        k,
    ) = Phi.shape  # gives the total number of rows and total columns of the matrix Phi
...
def append_scores(score_dict, source_dict):
    ...
    for k in source_dict:
        ...
        for idx, i in enumerate(score_dict[k].columns.tolist()):
            score_dict[k] = score_dict[k].rename(
                columns={i: "{}_fac{}".format(k, idx + 1)}
            )
...
def merge_dataframes(
    ...
    for k, v in input_dict.items():
        ...
        if idx == 1:
            ...
            merged_df = master_df.merge(
                input_dict[k][cols_to_use], on="observation_id", how="outer"
            )
    ...
    output_name = (
        data_path.split(".csv")[0]
        + (
            "_{results_id}_{vari}_ncomponents={ncomp}".format(
...
def page_of_plots(
    ...
    """
    out_pdf = (
        "scratch/results/figures_{results_id}_{vari}_ncomponents={ncomp}.pdf".format(
            results_id=results_id,
            vari=("varimax-on" if varimax_on else "varimax-off"),
            ncomp=(list(n_components_dict.values()) if n_components else "EV>1"),
        )
    ...
    for k in pca_dict:
        ...
        screeplt = sns.lineplot(
            ax=axes[0, 0], data=pca_dict[k].explained_variance_ratio_ * 100, marker="o"
        )
        screeplt.set_xticks(range(len(pca_dict[k].explained_variance_ratio_)))
        screeplt.set_xticklabels(
            range(1, len(pca_dict[k].explained_variance_ratio_) + 1)
        )
        ...
        cumplt = sns.lineplot(
            ax=axes[0, 1],
            data=np.cumsum(pca_dict[k].explained_variance_ratio_) * 100,
            marker="o",
        )
        ...
        y = loadings_dict[k][1, :]
        texts = [
            loadplt.text(x[i], y[i], display[k][i], ha="center", va="center")
...
def project_patterns(
    component_loadings_dict,
    master_df_with_scores,
    component_loadings_dict_key,
    condition_projecting,
    condition_to_project_on,
    sample_to_project_on,
    n_components,
    cond_col_str,
    sample_col_str,
    esq_Zstart,
    ...
    # select rows
    observations_to_project_on = master_df_with_scores.loc[
        master_df_with_scores[cond_col_str] == condition_to_project_on
    ]
    observations_to_project_on = observations_to_project_on.loc[
        master_df_with_scores[sample_col_str] == sample_to_project_on
    ]
    # select columns
    loc = master_df_with_scores.columns.get_loc
    observations_to_project_on = observations_to_project_on.iloc[
        :, np.r_[loc("observation_id"), loc(esq_Zstart) : loc(esq_Zend) + 1]
    ]
    # compute dot product
    ...
    projected_scores.columns = [
        f"{condition_projecting}_pattern_on_{condition_to_project_on}-{sample_to_project_on}_fac{x}"
...
def wordclouder(component_loading_dict, display, savefile=False):
    ...
    for (
        ...
        component_loading_dict.items()
    ):  # Loop over loading dictionaries - 1 dataframe per iteration
        df = pd.DataFrame(value.T)  # transpose
        output_name = "scratch/results/{}_component_loadings_for_wordclouds.csv".format(
            key
        )
        if savefile:
        ...
        vmin = -vmax  # minimu
        for i in range(
            principle_vector.shape[1]
        ):  # loop through each column (compoenent)
            ...
            pv_in_hex.append(
                colors_hex
            )  # add all colour codes for each item on all components
        colors_hex = np.array(pv_in_hex).T
        df_v_color = pd.DataFrame(colors_hex)
        if savefile:
            df_v_color.to_csv(
                "scratch/results/{}_colour_codes_for_wordclouds.csv".format(key),
                index=False,
                header=False,
            )
        else:
        ...
        for col_index in df:
            ...
            concat = pd.concat(
                [integer, df_v_color[col_index]], axis=1
            )  # concatanate loadings and colours
            concat.columns = ["freq", "colours"]
            concat.insert(
                1, "labels", display[key]
            )  # add labels (items) from display list
            if savefile:
                concat.to_csv(
                    "scratch/results/{}_loadings_and_colour_codes_factor_{}.csv".format(
                        key, col_index + 1
                    ),
                    index=False,
                    header=True,
                ...
                pass
            freq_dict = dict(
                zip(concat.labels, concat.freq)
            )  # where key: item and value: weighting
            colour_dict = dict(
                zip(concat.labels, concat.colours)
            )  # where key: itemm and value: colour
            ...
            wc.to_file(
                "scratch/results/{}_wordcloud_factor_{}.png".format(key, col_index + 1)
            )
